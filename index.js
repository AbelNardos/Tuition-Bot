...
bot.on('message:photo', async (ctx) => {
  ...
  try {
    ...
    // NEW: Download the image to the bot's memory first to bypass OCR.space URL blocks
    ...
    // 3. Ping the OCR.space API via a secure POST request
    ...
    // Failsafe: Detect if OCR.space rejected the request due to limits or downtime
    if (ocrData) {
      if (ocrData.IsErroredOnProcessing || ocrData.ErrorMessage) {
        apiFailed = true;
      } else if (ocrData.ParsedResults && ocrData.ParsedResults.length > 0) {
    ...
    // UPDATED: Broadened keyword array to catch all types of Ethiopian bank receipts
    const validKeywords = ['CBE', 'COMMERCIAL BANK', 'AWASH', 'TELEBIRR', 'BIRR', 'ETB', 'BOA', 'DEPOSIT', 'TRANSACTION', 'SUCCESS', 'COMPLETED', 'TRANSFER', 'NIB', 'DASHEN'];
    const hasKeyword = validKeywords.some(kw => extractedText.includes(kw));
    const hasDigits = (extractedText.match(/\d{4,}/) !== null); 
    
    // Fallback: If OCR API crashes, pass it to humans instead of incorrectly rejecting the student
    const isValid = apiFailed || hasKeyword || hasDigits;
